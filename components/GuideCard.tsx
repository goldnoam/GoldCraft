    ...
    <div className="bg-[#3d3d3d] rounded-lg overflow-hidden border-2 border-transparent hover:border-yellow-500 hover:scale-[1.03] hover:shadow-[0_20px_50px_rgba(255,215,0,0.1)] transition-all duration-300 group flex flex-col h-full shadow-2xl">
      <div className="relative h-48 overflow-hidden bg-[#2a2a2a]">
        ...
        <img 
          ...
          className={`w-full h-full object-cover transition-opacity duration-1000 ease-in-out ${imageLoaded ? 'opacity-100' : 'opacity-0'} group-hover:scale-110 transition-transform duration-500`}
        />
        ...
        <button 
          onClick={handleCopy}
          className="absolute bottom-2 left-2 p-2 bg-black/50 hover:bg-black/80 rounded-lg text-white transition-colors flex items-center gap-1 text-xs z-10"
          title="העתק תיאור"
        >
